...
def make_averaged(original_function, trials_count=1000):
    ...
    def average(*args):
        total_points = 0
        k = 0
        while trials_count != k:
            total_points += original_function(*args)
            k += 1
        return total_points / trials_count
    return average
    # END PROBLEM 8
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    num_rolls = 1
    highest_average = 0
    highest_averange_num = 0
    while num_rolls <= 10:
        current_average = make_averaged(dice, trials_count)
        if int(current_average()) > highest_average:
            highest_average = current_average
            highest_average_num = num_rolls
            num_rolls+=1
        else:
            num_rolls+=1

    return highest_average_num
...
def extra_turn_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    if bacon_strategy(score, opponent_score, cutoff, num_rolls) == 0:
        num_rolls = 0
    elif extra_turn(bacon_score, opponent_score):
        num_rolls = 0
